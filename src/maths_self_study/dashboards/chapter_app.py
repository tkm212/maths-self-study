...
from maths_self_study.viz.latex import latex_boot_script, latex_head_html
...
def create_chapter_dashboard(
    *,
    ...
    app.index_string = index.replace(
        "</head>",
        f"<style>{TAB_WRAP_STYLE}</style>{latex_head_html()}</head>",
    ).replace("</body>", f"{latex_boot_script()}</body>")
