...
- AFML Ch. 2–4 use Dash dashboards: `dashboard.py` plus a `fml_ch{N}_pages/` package per chapter. Each page is a subfolder with `filters.py`, `content.py`, and `callbacks.py`. Shared UI lives in `maths_self_study.dashboards`; dashboard entrypoint helpers live in `maths_self_study.demos.financial_machine_learning.dashboard`; plot helpers live in each chapter's `fml_ch{N}_helpers.py`; shared BTC data loaders in `maths_self_study.data`; AFML pipelines (bars → CUSUM → triple barrier → sample weights) in `maths_self_study.quant`; definitions, theorems, and observations in `maths_self_study.viz.textbooks.financial_machine_learning`.
