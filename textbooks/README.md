...
- ESL Ch. 2–6 use Dash dashboards: `dashboard.py` plus a `ch{N}_pages/` package per chapter (one tab per former Marimo notebook). Each page is a subfolder with `filters.py`, `content.py`, and `callbacks.py`. Shared UI lives in `maths_self_study.dashboards`; dashboard entrypoint helpers live in `maths_self_study.demos.elements_of_statistical_learning.dashboard`; plot and data helpers remain in each chapter’s `helpers.py` or `ch{N}_helpers.py`.
