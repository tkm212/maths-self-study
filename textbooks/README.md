...
- Deep Learning Ch. 2–7 use Dash dashboards: `dashboard.py` plus a `dl_ch{NN}_pages/` package per chapter. Each page is a subfolder with `filters.py`, `content.py`, and `callbacks.py`. Shared UI lives in `maths_self_study.dashboards`; dashboard entrypoint helpers live in `maths_self_study.demos.deep_learning.dashboard`; plot helpers live in each chapter's `dl_ch{NN}_helpers.py`; definitions and theorems in `maths_self_study.viz.textbooks.deep_learning`.
