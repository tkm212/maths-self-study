...
- Deep Learning Ch. 2–3 use Dash dashboards: `dashboard.py` plus a `ch{N}_pages/` package per chapter. Each page is a subfolder with `filters.py`, `content.py`, and `callbacks.py`. Shared UI lives in `maths_self_study.dashboards`. Other chapters import helpers from `maths_self_study.deep_learning` (`ch2_helpers` / `ch3_helpers`); local `ch{N}_helpers.py` files are thin re-exports.
