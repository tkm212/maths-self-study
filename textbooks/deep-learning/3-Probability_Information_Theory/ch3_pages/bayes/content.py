...
def _medical_table(prior_d: float, sens: float, fpr: float, posterior: np.ndarray) -> html.Div:
    return table(
        ["Quantity", "Value"],
        [
...
def render_body(prior_d, sens, fpr, chosen, opened) -> html.Div:
    ...
    if chosen_i == opened_i:
        ...
        return html.Div([
            ...
            graph(fig_med),
            _medical_table(prior_d, float(sens), float(fpr), posterior),
            html.H3("Monty Hall"),
    ...
    monty_rows = [[f"Door {i + 1} (win prob.)", f"{p:.3f}"] for i, p in enumerate(post)]
    return html.Div([
        ...
        graph(fig_monty),
        table(["Door", "P(win | evidence)"], monty_rows, caption="Posterior over doors"),
    ])
